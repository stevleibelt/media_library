 ...
 * @method GameQuery orderByGamePlatformId($order = Criteria::ASC) Order by the game_platform_id column
 *
 * @method GameQuery groupById() Group by the id column
 * @method GameQuery groupByMediaId() Group by the media_id column
 * @method GameQuery groupByGamePlatformId() Group by the game_platform_id column
 *
 ...
 *
 * @method GameQuery leftJoinGamePlatform($relationAlias = null) Adds a LEFT JOIN clause to the query using the GamePlatform relation
 * @method GameQuery rightJoinGamePlatform($relationAlias = null) Adds a RIGHT JOIN clause to the query using the GamePlatform relation
 * @method GameQuery innerJoinGamePlatform($relationAlias = null) Adds a INNER JOIN clause to the query using the GamePlatform relation
 *
 ...
 * @method Game findOneByGamePlatformId(string $game_platform_id) Return the first Game filtered by the game_platform_id column
 *
 ...
 * @method array findByGamePlatformId(string $game_platform_id) Return Game objects filtered by the game_platform_id column
 *
...
abstract class BaseGameQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, media_id, game_platform_id FROM net_bazzline_media_library_media_game WHERE id = :p0';
        try {
     ...
     * Example usage:
     * <code>
     * $query->filterByGamePlatformId('fooValue');   // WHERE game_platform_id = 'fooValue'
     * $query->filterByGamePlatformId('%fooValue%'); // WHERE game_platform_id LIKE '%fooValue%'
     * </code>
     *
     * @param     string $gamePlatformId The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return GameQuery The current query, for fluid interface
    ...
    public function filterByGamePlatformId($gamePlatformId = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($gamePlatformId)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $gamePlatformId)) {
                $gamePlatformId = str_replace('*', '%', $gamePlatformId);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(GamePeer::GAME_PLATFORM_ID, $gamePlatformId, $comparison);
    }
     ...
     *
     * @param   GamePlatform|PropelObjectCollection $gamePlatform The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 GameQuery The current query, for fluid interface
     * @throws PropelException - if the provided filter is invalid.
    ...
    public function filterByGamePlatform($gamePlatform, $comparison = null)
    {
        if ($gamePlatform instanceof GamePlatform) {
            return $this
                ->addUsingAlias(GamePeer::GAME_PLATFORM_ID, $gamePlatform->getId(), $comparison);
        } elseif ($gamePlatform instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }

            return $this
                ->addUsingAlias(GamePeer::GAME_PLATFORM_ID, $gamePlatform->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByGamePlatform() only accepts arguments of type GamePlatform or PropelCollection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Database\Media\GamePlatformQuery A secondary query class using the current class as primary query
     */
    public function useGamePlatformQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'GamePlatform', '\Database\Media\GamePlatformQuery');
    }
